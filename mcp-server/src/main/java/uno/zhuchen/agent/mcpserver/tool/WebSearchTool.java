 ...
 * 上网搜索 MCP 工具
 *
 * <p>通过云端搜索 API（百度 Qianfan / Tavily）检索互联网公开资料，返回结构化结果给 LLM。
 * <p>不抓取网页全文 — 仅返回搜索引擎摘要，避免 token 爆炸。
 * <p>降级策略：云端 API 失败时自动降级（百度→Tavily），均失败时返回友好提示。
 */
@Component
public class WebSearchTool {
    ...
    private final CloudSearchService searchService;
    private final ObjectMapper objectMapper;

    public WebSearchTool(CloudSearchService searchService, ObjectMapper objectMapper) {
        this.searchService = searchService;
        this.objectMapper = objectMapper;
    }

    @Tool(description = "上网搜索工具 — 通过云端搜索引擎检索互联网公开资料，返回 Top 10 条结果的标题、链接、摘要。"
    ...
    public String webSearch(
        ...
        List<CloudSearchService.SearchResult> results;
        try {
            results = searchService.search(query, n);
        } catch (Exception e) {
            log.error("搜索服务异常: query={}", query, e);
            results = List.of();
        }

        if (results == null) {
            log.warn("搜索服务返回 null: query={}", query);
            results = List.of();
        }

        // 降级: 云端搜索失败时返回友好提示,绝不抛异常
        if (results.isEmpty()) {
